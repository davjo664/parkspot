...
export function fetchLocations(searchString, userPosition) {
    const url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?location=${userPosition.latitude},${userPosition.longitude}
...
export function fetchLocationDetails(rowData) {
    ...
    const url = `https://maps.googleapis.com/maps/api/place/details/json?key=AIzaSyBtDPqZtRAMenSwz32oIUWWf1i_Gnub1dc&language=en&placeid=${rowData.place_id}`
    return dispatch =>
        fetch(url)
        .then(res => res.json())
        .then(data => {
            if (data.statusCode && data.statusCode != 200) {
                ...
                dispatch(fetchParkspots(data.result.geometry.location.lat, data.result.geometry.location.lng, 6000));
            }
        });
}